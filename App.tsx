...
function App() {
  ...
  const [lyrics, setLyrics] = useState<LyricData>(() => {
    ...
    if (savedLyrics) {
      try {
        ...
        console.error("Failed to parse saved lyrics from sessionStorage", e);
      }
    }
  ...
  useEffect(() => {
    if (currentLineIndex !== -1 && scrollContainerRef.current) {
      const currentLine = document.getElementsByClassName("is-current")[0];
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="max-w-4xl mx-auto">
              {lyrics.length === 0 && (
                <div className="text-center text-gray-500 mt-20">
                  No lyrics loaded. Import a file or add a line.
                </div>
              )}
        ...
        <div className="w-[400px] bg-black flex flex-col border-l border-gray-800 shadow-2xl z-10">
          ...
          <div className="p-4 flex-1 bg-dark text-gray-300 text-sm overflow-y-auto">
            <h3 className="text-primary font-bold mb-2 text-lg">Shortcuts</h3>
            <ul className="space-y-2 list-disc pl-4 text-gray-400">
              ...
              <li>Drag the progress bar in YouTube player to scrub.</li>
              <li>
                Use the "Duration" field for karaoke visual length
                (approximate).
              </li>
            </ul>
            <div className="mt-6 p-3 bg-blue-900/20 border border-blue-500/30 rounded text-blue-200">
              <p>
                <strong>Note:</strong> Time format is <code>MM:SS.mm</code>.
              </p>
              <p>You can only add lines when the video is paused.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-panel w-full max-w-3xl h-[80vh] rounded-xl shadow-2xl flex flex-col border border-gray-600">
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <FileJson className="text-primary" /> Generated JSON
              </h2>
              <button
